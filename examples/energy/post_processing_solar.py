...
The script:
- Produces the plot of the three testing sets during the first week
...
#Datasets path
TEST_NORMAL_DS_PATH = "/dataset/energy/100_51_101.csv"
TEST_RATING_DS_PATH = "/dataset/energy/100_Rating_51_101.csv"
TEST_PERCENTILE_DS_PATH = "/dataset/energy/100_Percentile_51_101.csv"
...
TEGDET_VARIANTS_RESULTS_PATH = "/script_results/energy/tegdet_variants_results_solar.csv"
TEGDET_PARAMS_SENSITIVITY_RESULTS_PATH = "/script_results/energy/tegdet_params_sensitivity_results_solar.csv"
...
def compare_testing_sets(cwd, n_obs):
    ...
    tests = [cwd+TEST_NORMAL_DS_PATH, cwd+TEST_RATING_DS_PATH, cwd+TEST_PERCENTILE_DS_PATH]

    for test in tests:

        test_set = pd.read_csv(test)['Usage'].head(len(dp_range))
        #Generate plot
        label = test.split('/')[-1]
        plt.plot(dp_range, test_set, label=label)
...
def generate_report_params_sensitivity(cwd,detector):
    ...
    tmw = df_grouped['time2window'].to_numpy()
    ...
    plot_3D("n_bins","n_obs_per_period","time2window (ms)",n_bins,n_obs,tmw)
...
if __name__ == '__main__':
    ...
    #Compare testing sets (first day of observations)
    compare_testing_sets(cwd, 48)
