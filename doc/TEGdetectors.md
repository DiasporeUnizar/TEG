...
The ```tegdet``` package depends on several well-known ```Python``` packages as shown in the diagram below:
 ...
 <img src="https://github.com/DiasporeUnizar/TEG/blob/master/doc/overview.png" width="1000">
...
| computeOutliers(baseline: numpy array of float, prediction: numpy array of float, sigLevel: int): numpy array of int | Computes the outliers based on the ```baseline``` distribution of the dissimilarities, the ```prediction```  and the significance level ```sigLevel``` |
